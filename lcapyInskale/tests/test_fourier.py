...
from lcapyInskale.fexpr import FourierDomainExpression
from lcapyInskale.expr import TimeDomainExpression
import unittest
...
class LcapyTester(unittest.TestCase):
    ...
    def test_fourier(self):
        ...
        self.assertEqual(TimeDomainExpression('x(t)').FT(),
                         FourierDomainExpression('X(f)'), "x(t)")
        self.assertEqual(TimeDomainExpression('5 * x(t)').FT(),
                         FourierDomainExpression('5 * X(f)'), "5 * x(t)")
    ...
    def test_inverse_fourier(self):
        ...
        self.assertEqual((f * 0 + 10).IFT(), 10
                         * DiracDelta(t), "0")
        self.assertEqual(FourierDomainExpression('V(f)').IFT(),
                         TimeDomainExpression('v(t)'), "V(f)")
    ...
    def test_fourier2(self):
        ...
        self.assertEqual((rect(t) * cos(2 * pi * t)).FT(),
                         sinc(f - 1) / 2 + sinc(f + 1) / 2)
